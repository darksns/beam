...
| agent | `extension/agent.js` | lives in the page: reads the structure, acts on the DOM |
...
`test/agent.test.js` runs 20 checks of the agent against a simulated ACF DOM:
...
tab has no layout, outline, `do` sequences and `wait`.
`test/hub.test.js` starts a real hub, pretends to be the extension, and checks
the handshake, WebSocket framing up to 200 KB payloads, and that an unauthorized
or page-originated request is refused.
